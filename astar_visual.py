...
def visualize():
    ...
    connec_y = []
    for p in path:
        connec_x.append(ajm[p[0]][p[1]]["x"])
        connec_y.append(ajm[p[0]][p[1]]["y"])

    df = pd.DataFrame()
    for row in ajm:
        for i in range(len(row)):
            df = df.append(pd.DataFrame.from_dict(row[i]).head(1), ignore_index=True)
    ...
    fig.add_trace(go.Scatter(x=df.x,
        ...
        dy=10,
        name="Nodes"))
    fig.add_trace(go.Scatter(x=connec_x, y=connec_y, mode="lines", name="Path"))
    fig.show()
